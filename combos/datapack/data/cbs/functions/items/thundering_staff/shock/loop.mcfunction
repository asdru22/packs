...
#offset x
execute store result score #rand cbs.dummy run random value -50..50
...
#raycast
execute facing entity @s feet run function cbs:items/thundering_staff/shock/light_loop
# iterate
scoreboard players remove #loop cbs.dummy 1
execute if score #loop cbs.dummy matches 1.. at @s if block ~ ~ ~ #cbs:raycast_ignore run function cbs:items/thundering_staff/shock/loop
