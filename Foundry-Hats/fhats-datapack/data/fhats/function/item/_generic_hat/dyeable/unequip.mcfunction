...
$data modify storage fhats:temp root.inventory append from entity @s Inventory[{id:"minecraft:leather_horse_armor",components:{"minecraft:custom_data":{fhats:{id:"$(id)"}}}}]

# If the list isn't empty, search through it
# The specified function path is run for each item found
execute if data storage fhats:temp root.inventory[-1] run function fhats:item/_generic_hat/dyeable/loop